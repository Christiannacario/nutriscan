...
const foodItems = [
    'apple', 'banana', 'chicken', 'egg', 'sugar', 'rice'
];
...
function Homepage() {
            ...
            reader.onload = function () {
                ...
                fetch(`https://api.clarifai.com/v2/models/${MODEL_ID}/versions/${MODEL_VERSION_ID}/outputs`, requestOptions)
                    ...
                    .then(result => {
                        ...
                        // Check if the concepts contain known food items
                        const containsFood = concepts.some(concept => foodItems.includes(concept.name.toLowerCase()));

                        if (!containsFood) {
                            setError('The image does not contain recognizable food items.');
                            setNutritionInfo('');
                        } else {
                        ...
                        // To Stop the scan effect on the uploaded image
    ...
    const displayNutritionInfo = (concepts) => {
        ...
        for (const [key, value] of Object.entries(nutritionData)) {
            ...
            const roundedPercentage = value.toFixed(2);
            items.push(
                <div key={key} className={`nutrition-item ${colorCategory}`}>
                    <div>{roundedPercentage}G</div>
                    <div>{key}</div>
                </div>
            );
        }
    ...
    const isNutritionHealthy = (nutrition) => {
        ...
        for (const [key, value] of Object.entries(nutrition)) {
            if (value < healthyThresholds[key] && value > healthyThresholds[key]) {
                return false;
            }
        }
    ...
    const calculateNutrition = (concepts) => {
        const nutritionMapping = {
            'apple': { calories: 100, protein: 0.26, fat: 0.17, carbs: 26.00 },
        ...
        <>
            ...
            <div className='container mt-3'>
                <form id="image-check-form" onSubmit={(e) => { e.preventDefault(); submitImageAndIdentifyFood(); }}>
                    {/* <h2 className="text-center mt-1 head-text">NutriAI Scan</h2> */}
